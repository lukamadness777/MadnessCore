package dev.lukamadness.madnesscore.client.config.draw.eye;
...
public class EyePartSelectScreen extends Screen {
    ...
    public EyePartSelectScreen(Screen parent, MadnessCoreConfig cfg, SkinRegion currentRegion) {
    ...
    private void renderCanvasAndRegions(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        if (hovered != null) {
            ctx.drawCenteredTextWithShadow(this.textRenderer,
                    hovered.label(), this.width / 2, canvasY + canvasSize + 4, 0xFFFFFF);
        }
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (clicked != null) {
            this.client.setScreen(new EyeDrawScreen(parent, cfg, clicked));
            return true;
        }
    ...
    public void close() {
        if (this.client != null) {
            this.client.setScreen(new EyeDrawScreen(parent, cfg, currentRegion));
        }
    }
}